...
def main():
    ...
    for name, cmd, extra in BASELINES:
        full = f"{cmd} {ARGS} {extra}".strip()
        print(f"\n{'='*50}")
        print(f"[casas] {name}")
        print(f"{'='*50}")
        ret = subprocess.run(full, shell=True, cwd=root, timeout=600)
        if ret.returncode != 0:
            print(f"\nFAILED: {name}")
            return 1
