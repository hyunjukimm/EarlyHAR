"""
Attn (ETSCM-style) baseline - segment-based early classification with entropy selection.
Uses fold data, pad_sequences, trains Attn, evaluates with earliness/F1/HM.
"""
import argparse
import logging
import os
...
from baselines.Attn import create_attn_model
from data_preprocessing.data_preprocess import pad_sequences


def setup_args():
    parser = argparse.ArgumentParser(description="Attn (ETSCM-style) baseline")
    ...
    parser.add_argument("--num_prefix_steps", type=int, default=None, help="Early inference: use first N segments")
...
def main():
    ...
    log_file = os.path.join("logs", f"attn_{args.dataset}.log")
    logging.basicConfig(
        level=logging.INFO,
    ...
    for fold_idx in range(args.k_fold):
        ...
        for epoch in range(args.epochs):
            ...
            for i in range(0, len(perm), args.batch_size):
                ...
                logits, _ = model(X_b, num_prefix_steps=None)
        ...
        with torch.no_grad():
            for i in range(0, len(X_test), args.batch_size):
                ...
                logits, tau = model(X_b, num_prefix_steps=args.num_prefix_steps)
                pred = logits.argmax(dim=1).cpu().numpy()
                y_pred.extend(pred.tolist())
                earliness_list.extend(tau.cpu().numpy().tolist())
        y_pred = np.array(y_pred)
        tau_mean = float(np.mean(earliness_list)) if earliness_list else 0.0
        earliness = 1.0 - tau_mean
    ...
    summary.to_csv(os.path.join(results_dir, "attn_kfold_summary.csv"))
    df.to_csv(os.path.join(results_dir, "attn_per_fold_results.csv"), index=False)
